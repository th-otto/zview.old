...
mnote_olympus_entry_get_value (MnoteOlympusEntry *entry, char *v, unsigned int maxlen)
{
	...
	case MNOTE_NIKON_TAG_EXPOSUREDIFF: {
		...
		CC (entry->components, 4, v, maxlen);
		vl =  exif_get_long (entry->data, entry->order);
		aa = (vl>>24)&0xff; bb = (vl>>16)&0xff; cc = (vl>>8)&0xff;
		snprintf (v, maxlen, "%.1f",  cc?(float)aa*((float)bb/(float)cc):0 );
		break;
	}
	...
	case MNOTE_OLYMPUS_TAG_FLASHDEVICE:
		...
		vs = exif_get_short(entry->data, entry->order);
		/* search for the tag */
		for (i = 0; (items[i].tag && (int)items[i].tag != (int)entry->tag); i++)
			;
		if (!items[i].tag) {
	...
	case MNOTE_NIKON_TAG_LENS:
		...
		CC (entry->components, 4, v, maxlen);
		{
			...
			dd = R2D(vr);
			snprintf (v, maxlen, "%ld-%ldmm 1:%3.1f - %3.1f",aa,bb,cc,dd);
		}
		break;
