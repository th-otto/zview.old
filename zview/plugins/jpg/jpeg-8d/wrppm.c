...
start_output_ppm (j_decompress_ptr cinfo, djpeg_dest_ptr dinfo)
{
  ...
  /* Emit file header */
  switch ((int)cinfo->out_color_space) {
  case JCS_GRAYSCALE:
...
calc_buffer_dimensions_ppm(j_decompress_ptr cinfo, djpeg_dest_ptr dinfo)
{
  ...
  if (cinfo->out_color_space == JCS_GRAYSCALE)
    dest->samples_per_row = (JDIMENSION)(cinfo->output_width * cinfo->out_color_components);
  else
  ...
  if (cinfo->quantize_colors || BITS_IN_JSAMPLE != 8 ||
    ...
    dest->pub.buffer = (*cinfo->mem->alloc_sarray)
      ((j_common_ptr) cinfo, JPOOL_IMAGE,
       (JDIMENSION)(cinfo->output_width * cinfo->output_components), (JDIMENSION) 1);
