...
read_row_callback(png_structp png_ptr, png_uint_32 row_number, png_int_t pass)
{
...
static void PNGCBAPI
write_row_callback(png_structp png_ptr, png_uint_32 row_number, png_int_t pass)
{
    ...
    png_int_t num_text)
{
   while (num_text > 0)
   {
      switch ((int)text_ptr[--num_text].compression)
      {
...
test_one_file(const char *inname, const char *outname)
{
   ...
   int pass;
   png_int_t bit_depth, color_type;
...
#else
   read_ptr =
       png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, 0, 0);
#endif
...
#else
   write_ptr =
       png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, 0, 0);
#endif
...
#  endif
   ...
   {
#ifdef PNG_WRITE_SUPPORTED
      png_set_write_status_fn(write_ptr, 0);
#endif
      png_set_read_status_fn(read_ptr, 0);
   }
   ...
   {
      png_int_t interlace_type, compression_type, filter_type;
...
# endif
...
#ifdef PNG_WRITE_UNKNOWN_CHUNKS_SUPPORTED
   {
      ...
      png_int_t num_unknowns = png_get_unknown_chunks(read_ptr, read_info_ptr,
          &unknowns);
...
# endif
   ...
   {
      png_unknown_chunkp unknowns;
      png_int_t num_unknowns = png_get_unknown_chunks(read_ptr, end_info_ptr,
          &unknowns);
...
main(int argc, char *argv[])
{
   ...
   {
	long ret;
	if ((ret = slb_pnglib_open(NULL)) < 0) {
        fprintf(stderr, "cannot load pnglib.slb: %ld\n", ret);
        exit(1);
	}
   }
#endif
   ...
   dummy_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, 0, 0);
