...
static int ApplyColorProfile(const WebPData *const profile, WebPDecBuffer *const rgba)
{
	...
	if (profile == NULL || rgba == NULL)
		return 0;
	if (profile->bytes == NULL || profile->size < 10)
		return 1;
...
static void DrawBackground(void)
{
	...
	if (kParams.draw_anim_background_color)
	{
		...
		glColor4f(GetColorf(kParams.bg_color, 16),	// BGRA from spec
				  GetColorf(kParams.bg_color, 8), GetColorf(kParams.bg_color, 0), GetColorf(kParams.bg_color, 24));
		glRecti(-1, -1, +1, +1);
		glPopMatrix();
	}
}
...
static void DrawBackgroundScissored(int window_x, int window_y, int frame_w, int frame_h)
{
...
static void HandleDisplay(void)
{
	...
	} else
	{
		...
		if (prev->dispose_method == WEBP_MUX_DISPOSE_BACKGROUND)
		{
			...
			DrawBackgroundScissored(prev->x_offset, prev->y_offset, prev->width, prev->height);
		}
		if (curr->blend_method == WEBP_MUX_NO_BLEND)
		{
			// We simulate no-blending behavior by first clearing the current frame
			// rectangle and then alpha-blending against it.
			DrawBackgroundScissored(curr->x_offset, curr->y_offset, curr->width, curr->height);
		}
	}
...
static void StartDisplay(const char *filename)
{
	...
	char title[4096 + 15];			// max linux file len + viewername string
...
int main(int argc, char *argv[])
{
	...
	for (c = 1; c < argc; ++c)
	{
		...
		} else if (!strcmp(argv[c], "-version"))
		{
			...
			printf("WebP Decoder version: %d.%d.%d\nWebP Demux version: %d.%d.%d\n",
				   ...
				   dec_version & 0xff, (dmux_version >> 16) & 0xff, (dmux_version >> 8) & 0xff, dmux_version & 0xff);
			return 0;
	...
	if (!ImgIoUtilReadFile(kParams.file_name, &kParams.data.bytes, &kParams.data.size))
	{
		goto Error;
	}
	...
	{
#ifdef WEBP_HAVE_QCMS
		if (!WebPDemuxGetChunk(kParams.dmux, "ICCP", 1, &kParams.iccp))
			goto Error;
...
#endif
	StartDisplay(kParams.file_name);
